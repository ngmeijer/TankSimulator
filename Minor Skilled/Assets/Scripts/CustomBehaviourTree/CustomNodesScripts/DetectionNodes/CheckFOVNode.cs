...
namespace CustomBehaviourTree.CustomNodesScripts.DetectionNodes
{
    ...
    public class CheckFOVNode : BehaviourNode
    {
        ...
        {
            _nodeState = IsTargetInsideFOV(blackboard, controller) ? NodeState.Success : NodeState.Failure;
        
            return _nodeState;
        }
    
        //TODO: Refactor node (related to GetRandomNavMeshPointNode. Both check if a point is inside FOV)
        private bool IsTargetInsideFOV(AIBlackboard blackboard, AIController controller)
        {
            _currentTarget = blackboard.InvestigationFocusPoint;
