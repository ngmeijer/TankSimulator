...
namespace TankComponents
{
    public class MoveComponent : TankComponent
    {
        ...
        [Header("Center of Mass (CoM)")] [SerializeField]
        ...
        {
            Debug.Assert(_tankRB != null,
                $"Tank rigidbody reference in MoveComponent ({gameObject.name}) is null. Drag into the inspector");
            Debug.Assert(_centerOfMass != null,
                $"Center of mass reference in MoveComponent ({gameObject.name}) is null. Drag into the inspector");
            Debug.Assert(_leftTrackRenderer != null,
                $"Left track renderer reference in MoveComponent ({gameObject.name}) is null. Drag into the inspector");
            Debug.Assert(_rightTrackRenderer != null,
        ...
        private void OnDrawGizmos()
        {
            ...
            foreach (var wheel in LeftTrackWheelColliders)
            {
                ...
                Handles.Label(wheel.transform.position - new Vector3(0, 0.25f, 0),
                    $"Motor torque: {wheel.motorTorque}");
            }
        ...
        public void RotateTank(float rotateInputValue)
        {
            ...
            switch (rotateInputValue)
            {
                ...
                case < 0:
                    ...
                    SetTorqueForTracks(LeftTrackWheelColliders,
                        -torquePerWheel * (_properties.SingleTrackTorqueMultiplier / 2));
                    SetTorqueForTracks(RightTrackWheelColliders,
                        torquePerWheel * _properties.SingleTrackTorqueMultiplier);
                    break;
                //Rotating to right
                case > 0:
        ...
        {
            _tankRB.AddForce(0, 0, -_componentManager.GetCurrentBarrelDirection().z * _properties.KickbackForce,
                ForceMode.Impulse);
        }
        ...
        private float CalculateTorque(float inputValue)
        {
            ...
            _currentTorque = _properties.MotorTorque.Evaluate(_movementData.RPM) *
                             _properties.GearRatios.Evaluate(_movementData.GearIndex) * FINAL_DRIVE_RATIO * inputValue;
